...
%% Get calib (needed for old m data)
% if m_flag
%     calib = {dir(fullfile(cd,'p*calib*.mat')).name};
%     CalibFile  = calib{end};
...
for f = 1:length(freqs)
    ...
    if freqs(f) == 0
        ...
        if m_flag
            datafiles = {dir(fullfile(cd,'p*click*.m')).name}
        end
    else
        datafiles = {dir(fullfile(cd,['p*',num2str(freqs(f)),'*.mat'])).name};
        if m_flag
            datafiles = {dir(fullfile(cd,['p*',num2str(freqs(f)),'*.m'])).name};
        end
    end
    ...
    for d = 1:length(datafiles)
        ...
        if m_flag
            all_trials = x.AD_Data.AD_All_V;
            fs_orig = x.AD_Data.SampleRate;
...
%     thresh(f) = cor_fit.c-y/cor_fit.b;
    ...
    %Rarely happens, Q410 TTS seemed to have some activity
    if thresh(f)<0
        thresh(f) = 5;
    end
